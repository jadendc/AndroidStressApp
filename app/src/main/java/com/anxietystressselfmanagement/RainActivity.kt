...
class RainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (exerciseType == null) {
            Log.e("RainActivity", "Exercise type is missing. Intent extras: ${intent.extras}")
            Toast.makeText(this, "An error occurred. Please restart the app.", Toast.LENGTH_SHORT).show()
            finish()
            return
        }
        Log.d("RainActivity", "Received exerciseType: $exerciseType")

        val cycles = intent.getIntExtra("selectedCycles", 1)

        // Initialize MediaPlayer for Rain sound
        ...
        // Display the Rain GIF as background
        val backgroundGifView: ImageView = findViewById(R.id.backgroundGif)
        Glide.with(this)
            .asGif()
            .load(R.raw.raingif) // Replace with Rain GIF resource
    ...
    private fun getGifResource(exerciseType: String): Int {
        return when (exerciseType) {
            ...
            else -> R.raw.raingif // Default GIF for RainActivity
        }
    }
