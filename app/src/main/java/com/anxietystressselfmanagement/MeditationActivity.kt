...
class MeditationActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (exerciseType == null) {
            Log.e("MeditationActivity", "Exercise type is missing. Intent extras: ${intent.extras}")
            Toast.makeText(this, "An error occurred. Please restart the app.", Toast.LENGTH_SHORT).show()
            finish()
            return
        }
        Log.d("MeditationActivity", "Received exerciseType: $exerciseType")

        val cycles = intent.getIntExtra("selectedCycles", 1)

        // Initialize MediaPlayer for meditation sound
        ...
        // Display the Meditation GIF as background
        val backgroundGifView: ImageView = findViewById(R.id.backgroundGif)
        Glide.with(this)
            .asGif()
            .load(R.raw.meditationgif) // Replace with Meditation GIF resource
        ...
        // Resize the exercise GIF programmatically
    ...
    private fun getGifResource(exerciseType: String): Int {
        return when (exerciseType) {
            ...
            else -> R.raw.meditationgif // Default GIF for MeditationActivity
        }
    }
